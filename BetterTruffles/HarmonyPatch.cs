...
internal partial class Mod {
  public class FarmAnimal_behaviors_Patch {
    public static IEnumerable<CodeInstruction> Transpile(IEnumerable<CodeInstruction> instructions, ILGenerator generator) {
        ...
        bool skipNextInstructions = false;
        for (int i = 0; i < codes.Count; i++)
        {
            if (skipNextInstructions)
            {
                skipNextInstructions = false;
                continue;
            }
            if (codes[i].opcode == OpCodes.Callvirt && codes[i].operand.ToString().Contains("ContainsKey"))
            {
                // Replace ContainsKey check with TryGetValue
                codes[i].opcode = OpCodes.Nop;
                codes[i + 1].opcode = OpCodes.Nop;
                codes[i + 2].opcode = OpCodes.Nop;
                codes[i + 3].opcode = OpCodes.Nop;
                skipNextInstructions = true;

                // Insert new code for objects.ContainsKey(key)
                yield return new CodeInstruction(OpCodes.Ldarg_1); // Load the location object
                yield return new CodeInstruction(OpCodes.Ldfld, typeof(GameLocation).GetField("objects")); // Load objects field
                yield return new CodeInstruction(OpCodes.Ldloc_S, "key"); // Load the key
                yield return new CodeInstruction(OpCodes.Callvirt, typeof(Dictionary<Vector2, Object>).GetMethod("ContainsKey")); // Call ContainsKey method
                yield return new CodeInstruction(OpCodes.Brtrue_S, codes[i].labels[0]); // Branch if objects.ContainsKey(key) is true

                // Insert new code for terrainFeatures.TryGetValue(key, out var terrainFeature)
                yield return new CodeInstruction(OpCodes.Ldarg_1); // Load the location object
                yield return new CodeInstruction(OpCodes.Ldfld, typeof(GameLocation).GetField("terrainFeatures")); // Load terrainFeatures field
                yield return new CodeInstruction(OpCodes.Ldloc_1); // Load the key
                yield return new CodeInstruction(OpCodes.Callvirt, typeof(Dictionary<Vector2, TerrainFeature>).GetMethod("TryGetValue")); // Call TryGetValue method
                yield return new CodeInstruction(OpCodes.Stloc_2); // Store the result in terrainFeature variable

                // Now check if terrainFeature is not null and is not Grass
                yield return new CodeInstruction(OpCodes.Ldloc_2); // Load terrainFeature
                yield return new CodeInstruction(OpCodes.Brfalse_S, codes[i].labels[0]); // Branch if terrainFeature is null
                yield return new CodeInstruction(OpCodes.Ldloc_2); // Load terrainFeature again
                yield return new CodeInstruction(OpCodes.Isinst, typeof(Grass)); // Check if terrainFeature is Grass
                yield return new CodeInstruction(OpCodes.Brtrue_S, codes[i].labels[0]); // Branch if terrainFeature is Grass
                yield return new CodeInstruction(OpCodes.Ldc_I4_0); // Load false
                yield return new CodeInstruction(OpCodes.Ret); // Return false

                // Insert the original branch instruction
                yield return codes[i];
            }
            else
            {
                yield return codes[i]; // Keep the original instruction
            }
        }
    }
